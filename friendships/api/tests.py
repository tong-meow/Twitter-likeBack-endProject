...
class FriendshipApiTests(TestCase):
    ...
    def test_follow(self):
        ...
        # 反向关注会创建新的数据
        before_count = FriendshipService.get_following_count(self.jesse.id)
        response = self.jesse_client.post(FOLLOW_URL.format(self.eliza.id))
        self.assertEqual(response.status_code, 201)
        after_count = FriendshipService.get_following_count(self.jesse.id)
    ...
    def test_unfollow(self):
        ...
        self.assertEqual(response.status_code, 400)
        # unfollow 成功
        self.create_friendship(from_user=self.eliza, to_user=self.jesse)
        before_count = FriendshipService.get_following_count(self.eliza.id)
        response = self.eliza_client.post(url)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data['deleted'], 1)
        after_count = FriendshipService.get_following_count(self.eliza.id)
        self.assertEqual(after_count, before_count - 1)
    ...
    def test_followers_pagination(self):
        ...
        for i in range(page_size * 2):
            follower = self.create_user('jesse_follower{}'.format(i))
            friendship = self.create_friendship(from_user=follower, to_user=self.jesse)
            friendships.append(friendship)
            if follower.id % 2 == 0:
                self.create_friendship(from_user=self.eliza, to_user=follower)

        url = FOLLOWERS_URL.format(self.jesse.id)
        ...
        # eliza has followed users with even id
        response = self.eliza_client.get(url)
        for result in response.data['results']:
    ...
    def test_followings_pagination(self):
        ...
        friendships = []
        for i in range(page_size * 2):
            following = self.create_user('jesse_following{}'.format(i))
            friendship = self.create_friendship(from_user=self.jesse, to_user=following)
            friendships.append(friendship)
            if following.id % 2 == 0:
                self.create_friendship(from_user=self.eliza, to_user=following)

        url = FOLLOWINGS_URL.format(self.jesse.id)
        self._paginate_until_the_end(url, 2, friendships)
        ...
        # jesse has followed all his following users
        ...
        response = self.jesse_client.get(url, {'created_at__gt': last_created_at})
        self.assertEqual(response.status_code, 200)
        ...
        for friend in new_friends:
            new_friendships.append(self.create_friendship(from_user=self.jesse, to_user=friend))
        response = self.jesse_client.get(url, {'created_at__gt': last_created_at})
        self.assertEqual(len(response.data['results']), 3)
